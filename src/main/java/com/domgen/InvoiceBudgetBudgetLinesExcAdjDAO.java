 ...
 */
public class InvoiceBudgetBudgetLinesExcAdjDAO extends BaseDAO implements ISelect, IDelete, IInsert, IUpdate, IStatusCodes
{
	...
	private static final String TABLE = "SCHDAYFILF.MBUNSBMT4";
	 ...
	 *
	 */
	public InvoiceBudgetBudgetLinesExcAdjDAO() 
	{
		super ();
	...
	{		
		...
		try
		{
			...
			theVO.setInvoiceYear (rs.getString(ATTRIBUTE_FIELDS[8]));
			...
			//format
			if(theVO.getInvoiceLineStatus() == null)
			{
				theVO.setAmountGross("n/a");
			}
			...
			if (rs.getString(ATTRIBUTE_FIELDS[16]) != null)
			{
			try {
			...
			if (theVO.getInvoiceLineStatus() == null)
			{
				theVO.setBudgetLineEditable("Y");
			}
			else if (theVO.getInvoiceLineStatus().equalsIgnoreCase(INVOICE_STATUS_EARLY)
					|| theVO.getInvoiceLineStatus().equalsIgnoreCase(INVOICE_STATUS_NEW_ENTRY)
					|| theVO.getInvoiceLineStatus().equalsIgnoreCase(INVOICE_STATUS_UW_REJECTION)
					|| theVO.getInvoiceLineStatus().equalsIgnoreCase(INVOICE_STATUS_UW_QUERY))
			{
				theVO.setBudgetLineEditable("Y");
			}
			else
			{
				theVO.setBudgetLineEditable("N");
			}
		}
