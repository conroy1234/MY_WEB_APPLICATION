...
/**
 * Data Access Object for the Nature maintenance file
 *  
 * @version	1.0
 * @author 	GATLANM on 09-Aug-05
 */
public class NatureDAO extends BaseDAO implements ISelect, IDelete, IInsert, IUpdate
{
	...
	private static final String TABLE = "SCHDAYFILF.MBNATURE";
	...
	{		
		NatureVO theVO = (NatureVO)someVO;
		
		try
		{
			// store key fields
			theVO.setNature (rs.getString(KEY_FIELDS[0]));
		...
		{
			logger.error("SQL Exception occurred - Project [MarketingBudgets] class [NatureDAO] method [storeFields] - theVO: " + theVO + " , ResultSet: " + rs);
	...
	public PreparedStatement populateStatementDelete(IBaseVO someVO, PreparedStatement ps)
	{
		...
		try
		{
			ps.setString(1, theVO.getNature());
		}
		catch (SQLException e)
		{
			logger.error("SQL Exception occurred - Project [MarketingBudgets] class [NatureDAO] method [populateStatementDelete] - theVO: " + theVO + " , PreparedStatement: " + ps);
	...
	public PreparedStatement populateStatementInsert(IBaseVO someVO, PreparedStatement ps)
	{
		...
		{
			logger.error("SQL Exception occurred - Project [MarketingBudgets] class [NatureDAO] method [populateStatementInsert] - theVO: " + theVO + " , PreparedStatement: " + ps);
	...
	public PreparedStatement populateStatementUpdate(IBaseVO someVO, PreparedStatement ps)
	{
		...
		try
		{
			...
			ps.setString(3, theVO.getNature()); // key
		}
		catch (SQLException e)
		{
			logger.error("SQL Exception occurred - Project [MarketingBudgets] class [NatureDAO] method [populateStatementUpdate] - theVO: " + theVO + " , PreparedStatement: " + ps);
