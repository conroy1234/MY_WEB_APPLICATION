...
/**
 * Data Access Object for the VIEW Unauthorised Invoices
 *  
 * @version	1.0
 * @author 	SIMMR on 13-Jul-05
 */
public class InvoiceBudgetDAO extends BaseDAO implements ISelect, IDelete, IInsert, IUpdate
{
	...
	private static final String TABLE = "SCHDAYFILF.MBUNAUTH2";
	...
	{		
		logger.debug("InvoiceBudgetDao.storeFields start");
		...
		{
			logger.error("SQL Exception occurred - Project [MarketingBudgets] class [InvoiceBudgetDAO] method [storeFields] - theVO: " + theVO + " , ResultSet: " + rs);
			
			e.printStackTrace();
		}
		logger.debug("InvoiceBudgetDao.storeFields end");
	}
	...
	public List findByCriteria (HashMap search, HashMap sort)
	{
		...
		try
		{
		logger.debug("InvoiceBudgetDao.findByCriteria start");
		if (search == null) search = new HashMap();
		if (sort == null) sort = new HashMap();
		
		logger.debug("InvoiceBudgetDao.findByCriteria search stmt:"+
				search.toString() + " sort stmt: " +
				sort.toString());
		
		   list = selectHelper.findByCriteria(search,sort);
		
		if (list == null)
		{
			logger.debug("InvoiceBudgetDao.findByCriteria return list NULL");	
		}
		else
		{
		//next ORIGINAL CODE
		...
		//END ORIGINAL CODE
		logger.debug("InvoiceBudgetDao.findByCriteria end with list size:" +
				list.size());
		}
		}
		catch (Exception e)
		{
			logger.error("InvoiceBudgetDAO.findByCriteria Exception: " +
					e);
			e.printStackTrace();
		}
		return list;
	...
	public List findByCriteria ()
	{
		...
		//return selectHelper.findByCriteria (null, null);
			list = selectHelper.findByCriteria (null, null);
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	public PreparedStatement populateStatementDelete(IBaseVO someVO, PreparedStatement ps)
	{
		...
		{
			logger.error("SQL Exception occurred - Project [MarketingBudgets] class [InvoiceBudgetDAO] method [populateStatementDelete] - theVO: " + theVO + " , PreparedStatement: " + ps);
	...
	public PreparedStatement populateStatementInsert(IBaseVO someVO, PreparedStatement ps)
	{
		...
		{
			logger.error("SQL Exception occurred - Project [MarketingBudgets] class [InvoiceBudgetDAO] method [populateStatementInsert] - theVO: " + theVO + " , PreparedStatement: " + ps);
	...
	public PreparedStatement populateStatementUpdate(IBaseVO someVO, PreparedStatement ps)
	{
		...
		{
			logger.error("SQL Exception occurred - Project [MarketingBudgets] class [InvoiceBudgetDAO] method [populateStatementUpdate] - theVO: " + theVO + " , PreparedStatement: " + ps);
