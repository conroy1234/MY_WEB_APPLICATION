...
/**
 * Data Access Object for the ransaction Type Maintenance File
 *  
 * @version	1.0
 * @author 	SIMMR on 04-Jul-05
 */
public class TransactionTypeDAO extends BaseDAO implements ISelect, IDelete, IInsert, IUpdate
{
	...
	private static final String TABLE = "SCHDAYFILF.MBTRANS";
	...
	{		
		TransactionTypeVO theVO = (TransactionTypeVO)someVO;
		
		try
		{
			// store key fields
			theVO.setTransactionType (rs.getString(KEY_FIELDS[0]));
		...
		{
			logger.error("SQL Exception occurred - Project [MarketingBudgets] class [TransactionTypeDAO] method [storeFields] - theVO: " + theVO + " , ResultSet: " + rs);
	...
	public PreparedStatement populateStatementDelete(IBaseVO someVO, PreparedStatement ps)
	{
		...
		try
		{
			ps.setString(1, theVO.getTransactionType());
		}
		catch (SQLException e)
		{
			logger.error("SQL Exception occurred - Project [MarketingBudgets] class [TransactionTypeDAO] method [populateStatementDelete] - theVO: " + theVO + " , PreparedStatement: " + ps);
	...
	public PreparedStatement populateStatementInsert(IBaseVO someVO, PreparedStatement ps)
	{
		...
		{
			logger.error("SQL Exception occurred - Project [MarketingBudgets] class [TransactionTypeDAO] method [populateStatementInsert] - theVO: " + theVO + " , PreparedStatement: " + ps);
	...
	public PreparedStatement populateStatementUpdate(IBaseVO someVO, PreparedStatement ps)
	{
		...
		try
		{
			...
			ps.setString(3, theVO.getTransactionType()); // key
		}
		catch (SQLException e)
		{
			logger.error("SQL Exception occurred - Project [MarketingBudgets] class [TransactionTypeDAO] method [populateStatementUpdate] - theVO: " + theVO + " , PreparedStatement: " + ps);
